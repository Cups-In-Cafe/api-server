package com.highgroup.highgroup.config.webSocket.config.handler;
...
public class StompHandler implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if (StompCommand.CONNECT == accessor.getCommand()) { // websocket 연결요청
            ...
            //accessor.setUser( Principal::getName );
            // Header의 jwt token 검증
            jwtTokenUtil.validateToken2(jwtToken);
            ...
            chatService.sendChatMessage(ChatMessage.builder().type("ENTER").roomId(roomId).sender(name).build());
            ...
            String name = Optional.ofNullable((Principal) message.getHeaders().get("simpUser")).map(Principal::getName).orElse("UnknownUser");
            chatService.sendChatMessage(ChatMessage.builder().type("QUIT").roomId(roomId).sender(name).build());
