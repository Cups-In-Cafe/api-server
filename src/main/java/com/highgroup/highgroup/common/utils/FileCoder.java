...
public class FileCoder {
    ...
    public static void encryptFile(String originalFile,String encryptFile) throws Exception {
        String str_key ="123456789abcdefg";
        // 128비트의 키
        //SecretKeySpec key = new SecretKeySpec(toBytes("696d697373796f7568616e6765656e61", 16), algorithm);
        SecretKeySpec key = new SecretKeySpec(str_key.getBytes(), algorithm);

        FileCoder coder = new FileCoder(key);
        File f = new File(originalFile);
        coder.encrypt(f, new File(encryptFile));
        f.delete();
    }
    public static void decryptFile(String encryptFile,String originalFile) throws Exception {
        String str_key ="123456789abcdefg";
        // 128비트의 키
        //SecretKeySpec key = new SecretKeySpec(toBytes("696d697373796f7568616e6765656e61", 16), algorithm);
        SecretKeySpec key = new SecretKeySpec(str_key.getBytes(), algorithm);
        ...
        FileCoder coder = new FileCoder(key);
        coder.decrypt(new File(encryptFile), new File(originalFile));
    }
    /**
     * <p>
     * 문자열을 바이트배열로 바꾼다.
